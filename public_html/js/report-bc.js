...
Qms.controller('ProposalListCtrl', function($scope, $filter, $http, $state, Proposal, FlashMessage) {
	...
	$scope.updateGlobRent = function(index) {
		...
		if($scope.proposal.purchaseopt == '1'){
			angular.forEach($scope.proposal.products, function(val, key) {
				gtotal = (gtotal + parseInt(val.weekly_rent));
			})

			if( $scope.proposal.hardware_discount!=""){
				if( $scope.proposal.residual!=""){
					gtotalpm = (parseInt(curr_month.title) * parseInt(gtotal)) - parseInt($scope.proposal.hardware_discount) - parseInt($scope.proposal.residual);
				}else{
			...
			if( $scope.proposal.residual!=""){
				if( $scope.proposal.hardware_discount!=""){
					...
					gtotalpm = (parseInt(curr_month.title) * parseInt(gtotal)) - parseInt($scope.proposal.residual);
				}
				gtotal =  parseInt(gtotalpm) / parseInt(curr_month.title)
			}
			
			$scope.level_weekly = " Weekly ";
